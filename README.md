...
> Java / Spring Boot 기반으로 JWT 인증·인가, 권한 분기, 예외 흐름을 **실무 기준으로 설계·구현한 백엔드 중심 개인 프로젝트**

### 🔹 한눈에 보는 핵심
- JWT Access / Refresh Token 인증 구조 직접 설계
- Spring Security 필터 단계와 Service 계층 권한 로직 분리
- 운영 환경을 가정한 예외 처리 및 트랜잭션 설계
- 최소 UI를 통해 인증·권한 정책의 실제 동작 검증
